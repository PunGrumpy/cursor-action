...
fails there. Do not reach for `pull_request_target` to work around it: that runs
with a writable token in the context of the fork's own code.
...
`exit-code` only reports whether the SDK call succeeded, so gate on the text
instead, and keep the text out of the script itself:
...
A model can always answer in a shape you did not ask for, so treat this as a
signal, not a gate you can rely on.
